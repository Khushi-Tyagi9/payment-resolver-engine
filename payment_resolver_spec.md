...
## Problem
Razorpay already reconciles bank and gateway internally (late authorization, webhook retries). What it cannot see is whether the *merchant's own system* correctly reflected that resolved status — evidenced by a real production bug where a successful Razorpay renewal never updated a WooCommerce order from pending to completed.
...
5. **Idempotency**: unique constraint on record_id in audit_log — each record claimed and processed exactly once
...
- Top row: metric cards — total processed, verified in sync, auto-corrected, recovered amount (highlighted), flagged, disputed
